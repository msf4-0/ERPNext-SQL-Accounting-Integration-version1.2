...
DOCTYPE = "sql stock adjustment"
DOCTYPE_URL_NAME = "ST_AJ"

controller = FormController(DOCTYPE_URL_NAME)

class sqlstockadjustment(Document):
	def on_trash(self):
		doc_no = self.name
...
def parse_doc(doc):
	...
	if data.get("stock_adjustment") is not None:
		for item in data.get("stock_adjustment"):
			detail = {
